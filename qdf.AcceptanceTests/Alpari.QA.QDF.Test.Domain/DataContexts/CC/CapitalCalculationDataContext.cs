...
namespace Alpari.QA.QDF.Test.Domain.DataContexts.CC
{
    ...
    {
        public CapitalCalculationDataContext(string connectionString)
        ...
        public DataTable GetPositionSnapshots(CapitalCalculationSnapshotParameters ccParameters, string connectionName)
        {
            ...
            result.Columns.Add("last_snapshot_time", typeof(string));
            result.PrimaryKey = new[] { result.Columns["snapshot_time_to_minute"] };
            ccParameters.QueryDateTime = ccParameters.StartTime;
            while (ccParameters.QueryDateTime <= ccParameters.EndTime)
            {
                var singlePositionQueryString = GetSinglePositionQueryString(ccParameters);
                try
                {
                    result.ImportRow(
                        SelectDataAsDataTable(singlePositionQueryString, 0,
                            ccParameters.QueryDateTime == ccParameters.StartTime).Rows[0]);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
                    Console.WriteLine(singlePositionQueryString);
                }
                ccParameters.QueryDateTime = ccParameters.QueryDateTime.AddMinutes(1);
            }
            return result;
        ...
        private static string GetSinglePositionQueryString(CapitalCalculationSnapshotParameters ccParameters)
        {
            return String.Format(
                ...
                "USE {5}; " +
                " SELECT" +
                "    sn_time.snapshot_time_to_minute," +
                "    sn_prev.database1_volume," +
                "    sn_prev.last_snapshot_time " +
                " FROM " +
                "  (SELECT COALESCE(@time, '') AS snapshot_time_to_minute) AS sn_time " +
                " LEFT JOIN " +
                "  (SELECT" +
                "      COALESCE(cc.Volume, 0) AS database1_volume," +
                "      COALESCE(cc.last_snapshot_time, '') AS last_snapshot_time" +
                "    FROM (SELECT" +
                "        p.Volume," +
                "        t.last_snapshot_time" +
                ...
                "      ORDER BY p.Id DESC) AS cc) AS sn_prev" +
                "    ON TRUE;"
