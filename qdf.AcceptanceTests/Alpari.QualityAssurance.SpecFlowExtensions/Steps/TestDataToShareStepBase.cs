...
namespace Alpari.QualityAssurance.SpecFlowExtensions.Steps
{
    ...
    public class TestDataToShareStepBase : StepCentral
    {
        ...
        {
            TestDataToShareFeature TestDataToShareFeature = (TestDataToShareFeature ) TestRunContext["TestDataToShareFeature"];
            TestDataToShareFeature.ScenarioCleanup();
            TestDataToShareFeature.ScenarioTearDown();
            TestDataToShareFeature.FeatureTearDown();
        }
        ...
        /// Real uses of this should have specific methods for setting up specific data templates and not rely on the name parameter
        /// THey should also set up data in subclasses of TestRunContext so that specific data types can be assigned to properties as write once read many
        ...
        /// <summary>
        /// This demonstrates that the testRunContext shouldn't be used in its default mode to store data that we don't want to change throughout the entire test run
