...
namespace Alpari.QualityAssurance.SpecFlowExtensions.Context
{
    ...
    /// TestRunContext allows test designers to store data templates, SUT connections, manager classes etc. across an entire test run
    /// This object should not be used for storing data relating to specific tests, steps or even features - the ScenarioContext should be used for most data that is created and accessed regularly throughout a test
    /// The properties <seealso cref="StaticTime"></seealso>  and <seealso cref="StaticRandom"></seealso> can be used to guarantee that 2 scenarios were run as prt of the same test run if so desired
    /// </summary>
    [Serializable]
    public class TestRunContext : Dictionary<string, Object> , ISerializable 
    {
        ...
        public static string StaticTime
        {
            get
            {
                return TestRunContext.Instance[Instantiated].ToString();
            }
        }
        ...
        public static string StaticRandom
        {
            get
            {
                return TestRunContext.Instance[RandomFileName].ToString();
            }
        }

        /// <summary>
        /// used internally to create <seealso cref="StaticRandom"></seealso> but is also a quick way to get a fairly random alphanumeric string 
        /// </summary>
        /// <returns></returns>
        ...
        private TestRunContext()
        {
            ...
            this.Add(Instantiated, now.ToString() + "  " + now.Ticks.ToString());
            this.Add(RandomFileName, GenerateRandomStringFromFileName());
        }
